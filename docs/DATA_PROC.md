...
### Ad-hoc exploration — `scripts/data_proc/plot_vdf_hermite.py`, `plot_vdf_rotation_hermite.py`
...
- `_draw_vdf_rotation_hermite(fig, ax_raw, ax_rotated, ax_hermite, reader, extractor, cid, ...)` — private: the per-cell three-panel drawing (raw VDF | rotated into local B frame | log-space Hermite spectra, 2D-reduced like `plot_cluster_hermite_spectra`) shared by the two public functions below, so both always show the *production* representation (`vdf_to_hermite_spectra_log` of the rotated VDF -- what `run_snapshot_pca.py`'s `"hermite"` mode consumes) and can't drift apart. Includes rotation-correctness diagnostics: angle(B,V) on the raw panel, density change on the rotated panel (should be near 0; a large shift signals a rotation bug). Replaced the old `plot_vdf_and_hermite_grid` (which showed the retired raw-linear, unrotated Hermite transform) and `plot_vdf_rotation_comparison`.
- `plot_vdf_rotation_hermite(reader, cellids, coords_re, labels, label="magnetosheath", ...)` — the three panels for one representative VDF of `label`, picked via `pick_cluster_representative_cellids`. Everything computed live for that single cell -- no saved `X_rotated.npy`/`X_hermite.npy` needed. Backs `scripts/data_proc/plot_vdf_rotation_hermite.py` (shares `pipeline_config.py`, unlike `plot_vdf_hermite.py`'s independent params, since it needs real ground-truth labels).
- `plot_vdf_rotation_hermite_grid(reader, cellids, coords_re, ...)` — same three panels, one row per manually picked cell (no labels involved). Backs `plot_vdf_hermite.py`'s box/coordinate selection.
...
- `draw_shue_boundaries(ax, shue_fit, show_r_mp_circle=True, show_subsolar_marker=False, lobe_r_min_re=None)` — magnetopause/bow-shock curves, plus two independent optional circles that are never conflated: `show_r_mp_circle` draws `R = r_mp` (the fitted Shue *magnetopause standoff distance*, always `shue_fit["r_mp_re"]`), and `lobe_r_min_re` (only drawn if given) draws the separate *inner_magnetosphere/lobes classification cutoff* -- pass the same value `classify_magnetosphere_regions` was actually called with, or the circle silently disagrees with the real split. Plus optional subsolar marker (at `r_mp`).
...
- `plot_som_label_maps(som_maps, output_path=None)` — per-blind-cluster SOM maps painted with expert labels (the SOM analogue of `plot_pca_scatter`'s `cluster_phys` coloring): one column per blind cluster, U-matrix row + codebook-KMeans-partition row, samples jittered on their best-matching unit with one shared label→color/marker mapping across panels. Takes plain arrays only (`ml_models.vdf_snapshot_clustering.fit_som_map`'s outputs plus `phys_labels`) -- no SOM objects, keeping the `data_proc`-never-imports-`ml_models` rule intact. Backs `scripts/ml_models/run_snapshot_som.py`.

### Cluster-plotting pipeline — `scripts/ml_models/run_snapshot_pca.py` + `plot_snapshot_pca.py`

Step 1 is `plot_colormap_with_vdf_markers` above; steps 2 and 3 close out the file. The same functions also back `scripts/data_proc/verify_data.py`, scored against real extraction ground truth instead of blind clusters:

- `_draw_cluster_positions(ax, ...)` — private: the spatial-position drawing logic (colormap + one marker per representative cellid) shared by the two public functions below, factored out so they can never visually disagree. Deliberately doesn't overlay the current-layer core dot cloud (`draw_current_layer_region`) -- this plot is about where representatives were pulled from, not detection extent; see `plot_combined_clusters` for that. Returns `(handles, labels)` rather than placing a legend itself, since `draw_snapshot_colormap`'s equal-aspect axes means the legend position has to be computed from the axes' actual rendered extent (`_place_legend_below`), not a fixed guess.
- `plot_cluster_vdf_positions(...)` — **pipeline step 2**, standalone: colormap with the spatial position of each cluster's representative VDF, via `_draw_cluster_positions`, legend placed underneath via `_place_legend_below`. Use this directly only when the positions plot is needed on its own; the combined verification figure below is what `verify_data.py`/`plot_snapshot_pca.py` actually save.
- `plot_cluster_vdf_examples(...)` — **pipeline steps 2+3 combined**: a header row with `_draw_cluster_positions`'s spatial-position plot (height sized from `boxre`'s own aspect ratio, not a fixed guess -- see its comment), followed by one example VDF per cluster below (three velocity-space projections per row, sliced through each VDF's own peak). One saved image instead of two separate `vdf_positions.png`/`vdf_examples.png` files.
